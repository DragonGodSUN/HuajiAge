...
public  class TileEntityHuajiBlader extends TileEntity implements IInventory, ITickable,ISidedInventory {
	...
	public TileEntityHuajiBlader() 
	{
	 ...
	 protected void setSlotsForExtract(int slot) {
		  ...
		  public boolean canExtractItem(int index, ItemStack stack, EnumFacing direction) {
		    return index !=36&&this.invHandler.canExtract(index);
		  }
		  @Override
	...
	public static ItemStack getSmeltingResultForItem(ItemStack stack) { return HuajiRecipeList.instance().getSmeltingResult(stack); }
 ...
 */
  ...
  public ITextComponent getDisplayName() {
	...
	public boolean isItemValidForSlot(int slotIndex, ItemStack itemstack) {
		if(slotIndex==FIRST_FUEL_SLOT){
			if(getItemBurnTime(itemstack)>0) {
			return true;}
			}
		else if(slotIndex==FIRST_INPUT_SLOT){
			if(!HuajiRecipeList.instance().getSmeltingResult(itemstack).isEmpty()) {
			return true;}
			}
		else if(slotIndex==FIRST_OUTPUT_SLOT){
			return true;
			}
		return false;
